...
class Email_controller extends Manager_Controller
{
    /**
    ...
    public function index()
    {
        ...
        $this->data['subview'] = 'manager/user/email/email_view';
    ...
    function sendmail()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($this->email->send()) {
                ...
                $this->data['subview'] = 'manager/user/email/email_success_view';
                $this->data['subview'] = 'manager/user/email/email_view';
                $this->load->view("manager/_layout_main", $this->data);
            } else {
                echo '<script>alert("Error occurred while sending the email.Please check your internet connection!");</script>';
                $this->data['subview'] = 'manager/user/email/email_unsuccess_view';
                $this->data['subview'] = 'manager/user/email/email_view';
                $this->load->view("manager/_layout_main", $this->data);
            }
            return;
